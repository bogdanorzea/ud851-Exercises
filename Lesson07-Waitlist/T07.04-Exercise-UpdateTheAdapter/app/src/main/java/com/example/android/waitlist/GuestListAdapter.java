...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        ...
        int indexColumnGuestName = mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_GUEST_NAME);
        int indexColumnPartySize = mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_PARTY_SIZE);

        // COMPLETED (6) Call getString on the cursor to get the guest's name
        String guestName = mCursor.getString(indexColumnGuestName);

        // COMPLETED (7) Call getInt on the cursor to get the party size
        int partySize = mCursor.getInt(indexColumnPartySize);
